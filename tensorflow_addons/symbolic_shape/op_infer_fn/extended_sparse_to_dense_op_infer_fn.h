...
class ExtendedSparseToDenseOpInferFn : public SymbolicShapeFn {
  bool
  InferExtendedSparseToDenseShape(std::shared_ptr<SymbolicShapeContext> context,
                                  NodeDef *node);

public:
  ExtendedSparseToDenseOpInferFn() = default;

  bool Infer(std::shared_ptr<SymbolicShapeContext> context,
             NodeDef *node) override {
    RETURN_IF_FALSE(InferExtendedSparseToDenseShape(context, node));
    return true;
  }
};
