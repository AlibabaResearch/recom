...
class UniqueOpInferFn : public SymbolicShapeFn {
  bool InferUniqueShape(std::shared_ptr<SymbolicShapeContext> context,
                        NodeDef *node);

public:
  UniqueOpInferFn() = default;

  bool Infer(std::shared_ptr<SymbolicShapeContext> context,
             NodeDef *node) override {
    RETURN_IF_FALSE(InferUniqueShape(context, node));
    return true;
  }
};
