...
class SparseFillEmptyRowsOpInferFn : public SymbolicShapeFn {
  bool ConstrainInput(std::shared_ptr<SymbolicShapeContext> context,
                      NodeDef *node);
  bool
  InferSparseFillEmptyRowsShape(std::shared_ptr<SymbolicShapeContext> context,
                                NodeDef *node);

public:
  SparseFillEmptyRowsOpInferFn() = default;

  bool Infer(std::shared_ptr<SymbolicShapeContext> context,
             NodeDef *node) override {
    if (!ConstrainInput(context, node)) {
      RECOM_VLOG << "Fail to constrain the input of SparseFillEmptyRows op";
    }
    RETURN_IF_FALSE(InferSparseFillEmptyRowsShape(context, node));
    return true;
  }
};
