...
class SelectOpInferFn : public SymbolicShapeFn {
  bool ConstrainInput(std::shared_ptr<SymbolicShapeContext> context,
                      NodeDef *node);

  bool InferSelectShape(std::shared_ptr<SymbolicShapeContext> context,
                        NodeDef *node);

public:
  SelectOpInferFn() = default;

  bool Infer(std::shared_ptr<SymbolicShapeContext> context,
             NodeDef *node) override {
    if (!ConstrainInput(context, node)) {
      RECOM_VLOG << "Fail to constrain Select input";
    }

    RETURN_IF_FALSE(InferSelectShape(context, node));
    return true;
  }
};
