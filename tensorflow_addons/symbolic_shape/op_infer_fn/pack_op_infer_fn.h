...
class PackOpInferFn : public SymbolicShapeFn {
  bool ConstrainInput(std::shared_ptr<SymbolicShapeContext> context,
                      NodeDef *node);

  bool InferPackShape(std::shared_ptr<SymbolicShapeContext> context,
                      NodeDef *node);

  bool InferPackContent(std::shared_ptr<SymbolicShapeContext> context,
                        NodeDef *node);

public:
  PackOpInferFn() = default;
  ...
  bool Infer(std::shared_ptr<SymbolicShapeContext> context,
    ...
    if (!InferPackShape(context, node)) {
      RECOM_VLOG << "Fail to infer Pack shape";
      shape_flag = false;
    }
    if (!InferPackContent(context, node)) {
      RECOM_VLOG << "Fail to infer Pack content";
    }
    return shape_flag;
  }
};
