...
class ScatterNdOpInferFn : public SymbolicShapeFn {
  bool InferScatterNdShape(std::shared_ptr<SymbolicShapeContext> context,
                           NodeDef *node);

public:
  ScatterNdOpInferFn() = default;

  bool Infer(std::shared_ptr<SymbolicShapeContext> context,
             NodeDef *node) override {
    RETURN_IF_FALSE(InferScatterNdShape(context, node));
    return true;
  }
};
