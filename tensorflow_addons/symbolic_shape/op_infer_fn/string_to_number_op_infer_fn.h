...
class StringToNumberOpInferFn : public SymbolicShapeFn {
  bool InferStringToNumberShape(std::shared_ptr<SymbolicShapeContext> context,
                                NodeDef *node);

public:
  StringToNumberOpInferFn() = default;

  bool Infer(std::shared_ptr<SymbolicShapeContext> context,
             NodeDef *node) override {
    RETURN_IF_FALSE(InferStringToNumberShape(context, node));
    return true;
  }
};
