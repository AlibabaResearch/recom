...
class StringToHashBucketFastOpInferFn : public SymbolicShapeFn {
  bool InferStringToHashBucketFastShape(
      std::shared_ptr<SymbolicShapeContext> context, NodeDef *node);

public:
  StringToHashBucketFastOpInferFn() = default;

  bool Infer(std::shared_ptr<SymbolicShapeContext> context,
             NodeDef *node) override {
    RETURN_IF_FALSE(InferStringToHashBucketFastShape(context, node));
    return true;
  }
};
