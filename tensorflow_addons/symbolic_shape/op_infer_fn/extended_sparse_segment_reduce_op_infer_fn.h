...
class ExtendedSparseSegmentReduceOpInferFn : public SymbolicShapeFn {
  bool InferExtendedSparseSegmentReduceShape(
      std::shared_ptr<SymbolicShapeContext> context, NodeDef *node);

public:
  ExtendedSparseSegmentReduceOpInferFn() = default;

  bool Infer(std::shared_ptr<SymbolicShapeContext> context,
             NodeDef *node) override {
    RETURN_IF_FALSE(InferExtendedSparseSegmentReduceShape(context, node));
    return true;
  }
};
