...
#include "gather_value_gen_indice_ops.h"

namespace tensorflow {
namespace feature_opt {

template <typename T>
GatherValueGenIndiceOp<T>::GatherValueGenIndiceOp(OpKernelConstruction *c)
    : OpKernel(c) {
...
template <typename T>
void GatherValueGenIndiceOp<T>::Compute(OpKernelContext *c) {
  ...
  OP_REQUIRES_OK(c, c->allocate_output(0, {num_output, 1}, &output_indices_t));
...
REGISTER_KERNEL_BUILDER(Name("Addons>GatherValueGenIndice")
                            .Device(tensorflow::DEVICE_CPU)
                            .TypeConstraint<int32>("T"),
                        GatherValueGenIndiceOp<int>);

REGISTER_KERNEL_BUILDER(Name("Addons>GatherValueGenIndice")
                            .Device(tensorflow::DEVICE_CPU)
                            .TypeConstraint<int64>("T"),
                        GatherValueGenIndiceOp<int64>);

REGISTER_OP("Addons>GatherValueGenIndice")
    .Input("values: T")
